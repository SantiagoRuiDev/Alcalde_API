...
export const crearArticulo = async (data) => {
    const connection = await connectDatabase();
    const [result] = await connection.query('INSERT INTO articulos(id_usuario, titulo, subtitulo, descripcion, imagen) VALUES (?, ?, ?, ?, ?)', [data.id_usuario, data.titulo, data.subtitulo, data.descripcion, data.imagen]);
